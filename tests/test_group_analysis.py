...
def mock_sig_of_relationship(self, dimension, question, test_type):
...
def sig_of_relationship_wrapper(dimension, question, test_type):
    return mock_sig_of_relationship(None, dimension, question, test_type)
...
def test_use_alternate_test_type(monkeypatch):
    monkeypatch.setattr(
        Processor, 'all_questions', mock_all_questions)
    monkeypatch.setattr(
        Processor, 'all_dimensions', mock_all_dimensions)

    exp_c = ['dimension', 'question', 'p_value', 'test_type']
    exp_r = [
        ('D1', 'Q1', 0.01, 'linearmodel'),
        ('D2', 'Q1', 0.02, 'linearmodel'),
        ('D2', 'Q2', 0.045, 'linearmodel'),
        ('D1', 'Q2', 0.1,  'linearmodel'),
    ]
    exp_df = pd.DataFrame.from_records(
        exp_r, columns=exp_c).set_index(['dimension', 'question'])
    ...
    with patch.object(Analyzer,
                      ...
                      side_effect=sig_of_relationship_wrapper) as sor_mock:
        p = Processor()
        res = GroupAnalysis(processor=p).significant_relationships_list(
            None, None, 'linearmodel')
    assert_frame_subset(res, exp_df)
    sor_mock.assert_called_with('D2', 'Q2', 'linearmodel')
